...
async function rightCode(friend) {
  ...
  await sleep(350);
  await typeLine("BONJOUR " + friend.name.toUpperCase() + " 👋", "ok", 38);
  await sleep(650);
  await typeLine("⚠ ANOMALIE DÉTECTÉE DANS LE SYSTÈME", "err", 22);
  await sleep(250);
  await typeLine("Regarde bien la photo.", null, 26);
